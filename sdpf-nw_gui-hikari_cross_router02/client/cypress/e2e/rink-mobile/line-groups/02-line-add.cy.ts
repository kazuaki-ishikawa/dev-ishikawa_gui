...
const ADD_LINE_NUMBER = '0000000002'
const NOT_EXISTING_LINE_NUMBER = '0000000003'

describe('容量シェアグループ内回線追加テスト', () => {
  beforeEach(function () {
    ...
    cy.intercept('PUT', '**/rink-mobile/v1/tenants/*/line-groups/line-members/*', {
      body: { id: generateRandomHex(32) },
    }).as('putRinkLineGroupLineMembers')
  ...
  it('回線追加：入力フォーム', function () {
    ...
    // 容量シェアグループを選択し、回線追加画面へ遷移する
    cy.get('[data-cy="rink-mobile-line-groups-index-rink-line-table"]').find('.radio .button').first().click()
    cy.get('[data-cy="rink-mobile-line-groups-index-add-line-button"]')
      .should('have.text', t('rinkLineGroups.addLine'))
      .click()
    cy.wait(['@getRinkLineGroupListMax', '@getAllRinkLineList'])

    // メッセージ非表示確認
    cy.get('[data-cy="rink-mobile-line-groups-id-add-lines-outside-reception-hour"]').should('not.exist')
    ...
    // ボタン初期値
    cy.get('[data-cy="rink-mobile-line-groups-id-add-lines-cancel-button"]')
      .should('have.text', t('common.cancel'))
      .and('not.be.disabled')
    cy.get('[data-cy="rink-mobile-line-groups-id-add-lines-submit-button"]')
      .should('have.text', t('common.confirm'))
      .as('submitButton')
      .and('be.disabled')

    // 既にグループに追加されている回線は選択できないことを確認する
    ...
    cy.get('[data-cy="edit-rink-line-line-table"]')
      .find(`[data-cy="rink-line-table-checkbox-${ADD_LINE_NUMBER}"]`)
      .click()
    cy.get('[data-cy="edit-rink-line-add-button"]').click()
    ...
    // 回線の追加
    cy.get('@submitButton').should('have.text', t('common.add')).click()
    cy.wait('@putRinkLineGroupLineMembers').then(req => {
      expect(req.request.url).to.include(`/line-groups/line-members/${LINE_GROUP_ID}`)
      expect(req.request.body).to.deep.equals({ linesList: [{ lineIndex: 1, lineNumber: ADD_LINE_NUMBER }] })
    })
  ...
  it('回線追加：入力フォーム(回線数MAX:254の場合)', function () {
    const MAX_LINE_NUMBER = 254
    const IGNORE_LINE_NUMBER = `test${MAX_LINE_NUMBER - 2}`
    const lineList = Array.from({ length: MAX_LINE_NUMBER + 2 }, (_, i) => ({
      lineNumber: `test${i.toString().padStart(3, '0')}`,
      lineGroupId: i === MAX_LINE_NUMBER - 2 ? LINE_GROUP_ID : null,
    }))
    cy.intercept('GET', '**/rink-mobile/v1/tenants/*/lines/self-add/*', {
    ...
    // 全選択
    cy.get('[data-cy="edit-rink-line-line-table"]').find('[data-cy="rink-line-table-checkbox-all"]').click()
    cy.get('[data-cy="edit-rink-line-add-button"]').click()

    // 容量シェアグループ内の回線一覧 テーブルの確認
    cy.get('[data-cy="edit-rink-line-current-line-table"]')
      .find('.row')
      .should('have.length', MAX_LINE_NUMBER + 1)
    // 回線一覧 テーブルの確認
    cy.get('[data-cy="edit-rink-line-line-table"]')
    ...
    // 確認ボタンを押下する
    cy.get('[data-cy="rink-mobile-line-groups-id-add-lines-submit-button"]').as('submitButton').click()

    // 確認画面では追加する回線の情報のみ表示する
    cy.get('[data-cy="edit-rink-line-confirmation-line-table"]').find('.row').should('have.length', MAX_LINE_NUMBER)
    cy.get('[data-cy="edit-rink-line-confirmation-line-table"]')
      .find(`[data-cy="rink-line-table-checkbox-${IGNORE_LINE_NUMBER}"]`)
      .should('not.exist')
    ...
    cy.wait('@putRinkLineGroupLineMembers').then(req => {
      ...
      expect(req.request.body).to.deep.equals({
        linesList: lineList
          .filter(({ lineNumber }, index) => lineNumber !== IGNORE_LINE_NUMBER && index <= MAX_LINE_NUMBER)
  ...
  it('回線追加：CSVファイル', function () {
    ...
    // 容量シェアグループを選択し、回線追加画面へ遷移する
    cy.get('[data-cy="rink-mobile-line-groups-index-rink-line-table"]').find('.radio .button').first().click()
    cy.get('[data-cy="rink-mobile-line-groups-index-add-line-button"]').click()
    cy.wait(['@getRinkLineGroupListMax', '@getAllRinkLineList'])
    ...
    cy.editApplicationFileUpload({
      ...
      filePath: 'cypress/fixtures/rink-mobile/line-groups/line-add.csv',
    })
    ...
    // CSVファイルから追加した回線が表示されていることを確認する
    cy.get('[data-cy="edit-rink-line-current-line-table"]')
      .find(`[data-cy="rink-line-table-checkbox-${ADD_LINE_NUMBER}"]`)
      .should('exist')
    // 存在しない回線は追加されないことを確認する
    cy.get('[data-cy="edit-rink-line-current-line-table"]')
  ...
  context('受付時間外メッセージの表示を確認', function () {
    it('受付時間外 - 日本時間(20:00)', function () {
      ...
      // 容量シェアグループ内回線追加画面に直接遷移
      cy.visit(
        `/tenants/${this.tenantId}/rink-mobile/line-groups/${LINE_GROUP_ID}/add-lines?rinkMobileId=${RINK_MOBILE_ID_MAX}`,
      )
      cy.wait(['@getRinkLineGroupListMax', '@getAllRinkLineList'])

      // メッセージ表示確認
      cy.get('[data-cy="rink-mobile-line-groups-id-add-lines-outside-reception-hour"]').should('exist')
      // 追加する回線を選択し、確認画面に遷移
      cy.get('[data-cy="edit-rink-line-line-table"]')
        .find(`[data-cy="rink-line-table-checkbox-${ADD_LINE_NUMBER}"]`)
        .click()
      cy.get('[data-cy="edit-rink-line-add-button"]').click()
      // 追加する回線を選択後も確認ボタンが押せないことを確認する
      cy.get('[data-cy="rink-mobile-line-groups-id-add-lines-submit-button"]').should('be.disabled')
    })
