...
const {
  ...
  execute: getSession,
} = useAsyncData('getSession', () => getCasvalSession({ tenant_id: tenantId.value }), { immediate: false })
...
const moveToSummary = () => {
  return navigateTo(`/tenants/${tenantId.value}/monitoring/summary`, { replace: true })
}
  ...
  <div class="pa-5">
    ...
    <template v-if="useStub">
      <div v-for="tenant in tenantList" :key="tenant.tenantId">
        <NuxtLink :href="`/?tenant_id=${tenant.tenantId}`" class="tenant-link">
...
.tenant-link {
  color: #fff;
  &:hover {
    color: rgb(var(--v-theme-light-primary));
  }
}
</style>
